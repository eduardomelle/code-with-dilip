...
public class GreetingStreamsTopology {
    ...
    public void process(StreamsBuilder streamsBuilder) {
        var greetingsStream = streamsBuilder.stream(GREETINGS, Consumed.with(Serdes.String(), new JsonSerde<Greeting>()));
        greetingsStream.print(Printed.<String, Greeting>toSysOut().withLabel("greetingsStream"));

        var modifiedStream = greetingsStream.mapValues((readOnlyValue, value) ->
        ...
        modifiedStream.to(GREETINGS_OUTPUT, Produced.with(Serdes.String(), new JsonSerde<>()));
    }
